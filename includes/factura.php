...
class factura extends db implements crud {
    ...
    public static function totalEstadoDeCuentaPropiedad($inmueble, $apto) {
        $consulta = "select meses_pendiente as recibos, deuda_total as saldo from propiedades where id_inmueble='$inmueble' and apto='$apto'";
        return db::query($consulta);
    }
    public static  function estadoDeCuentaDetallado($inmueble,$apto) {
        $consulta = "select f.*,d.* from ".self::tabla. " f join factura_detalle d on f.numero_factura = d.id_factura where f.id_inmueble='".$inmueble."' and f.apto='".$apto.
                "' order by periodo ASC";
        return db::query($consulta);
    }
